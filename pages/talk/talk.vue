<template>
	<view>
		<view class="tips color-fff size_12 align-c" :class="{ 'show':ajax.loading }" @tap="getHistoryMsg">
			{{ajax.loadText}}
		</view>
		<view class="box-1" id="list-box">
			<view class="talk-list">
				<view v-for="(item,index) in talkList" :key="item.id" :id="'msg-'+item.id" @click='testFunc' :ref="'msg-'+item.id">
		...
		methods: {
			...
			testFunc: (event) => {
				if (event) {
					...
					console.log(text);
					let audioPlayer = uni.createInnerAudioContext();
